...
def tweet_to_csv(tweet_data):
    with open(f'twstalker_result_{time.time()}.csv', 'a+', newline='', encoding='utf-8') as f:
...
if __name__ =="__main__":
    ...
    with open('twitter_usernames.txt') as file:
        for twitter_id in file:
            ...
            url = f'https://www.twstalker.com/{twitter_id.strip()}/'
            ...
            if "404" != res.text:
                ...
                try:
                    tweet_text = twitter_id.strip()+r"/status/\d+"
                    found = re.findall(tweet_text, res.text)
                except AttributeError:
                    found = ""
                    print("[!] no tweet found")
                for tweets in found:
                    tweet_url = 'https://www.twstalker.com/'+tweets
                    #print(tweet_url)
